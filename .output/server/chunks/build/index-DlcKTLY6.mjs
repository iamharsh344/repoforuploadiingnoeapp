...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen text-white game-body" }, _attrs))}>`);
  ...
  ssrRenderList($data.displayedItems, (item) => {
    _push(`<div class="item-card rounded-xl p-6 relative rarity-rare"><div class="absolute top-3 right-3"><span class="category-badge text-xs px-2 py-1 rounded-full font-semibold">${ssrInterpolate(item.category == "crops" ? "Plants" : item.category.charAt(0).toUpperCase() + item.category.slice(1))}</span></div><div class="text-center mb-4"><div class="w-24 h-24 mx-auto mb-3 rounded-full flex items-center justify-center"><img${ssrRenderAttr("alt", item.imageName)}${ssrRenderAttr("src", `https://storage.silverfoxvalues.com/gag/${item.imageName}`)} loading="lazy"></div><h3 class="text-lg font-bold text-white">${ssrInterpolate(item.name)}</h3><p class="text-sm text-gray-400">${ssrInterpolate(item.category == "crops" ? "Plants" : item.category.charAt(0).toUpperCase() + item.category.slice(1))}</p></div><div class="space-y-2 mb-4"><div class="flex justify-between items-center"><span class="text-gray-300 text-sm">Price:</span><span class="value-badge text-sm px-2 py-1 rounded-full font-semibold flex gap-1"><img alt="Sheckle" width="20" height="20" class="w-[20px] h-[20px]"${ssrRenderAttr("src", _imports_0)}> ${ssrInterpolate(item.price.toUpperCase())}</span></div><div class="flex justify-between items-center"><span class="text-gray-300 text-sm">Robux:</span><span class="text-base px-2 py-1 rounded-full font-semibold">${ssrInterpolate(item.robux)}</span></div><div class="flex justify-between items-center"><span class="text-gray-300 text-sm">Multi-Harvest:</span><span class="text-base px-2 py-1 rounded-full font-semibold">${ssrInterpolate(item.multiHarvest.toUpperCase())}</span></div><div class="flex justify-between items-center"><span class="text-gray-300 text-sm">Demand:</span><span class="text-base px-2 py-1 rounded-full font-semibold">${ssrInterpolate(item.demand)}</span></div><div class="flex justify-between items-center"><span class="text-gray-300 text-sm">Trend:</span><span class="${ssrRenderClass([$options.getTrendClass(item.trend), "flex items-center"])}"><svg class="${ssrRenderClass([$options.getTrendIcon(item.trend), "mr-1 w-6 h-6"])}" viewBox="0 0 24 24" fill="currentColor"><path${ssrRenderAttr("d", $options.getTrendIconPath(item.trend))}></path></svg> ${ssrInterpolate(item.trend)}</span></div></div><button class="details-btn w-full py-2 rounded-lg font-semibold text-white"> View Details </button></div>`);
  });
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/growagarden/plants/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const index = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);

export { index as default };
//# sourceMappingURL=index-DlcKTLY6.mjs.map
