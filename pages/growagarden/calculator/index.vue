<template>
    <div class="bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950">
        ...
        <div class="max-w-6xl mx-auto pt-44 pb-20">
          <h1 class="font-bold sm:text-5xl text-3xl text-center bg-gradient-to-r from-emerald-400 to-blue-400 bg-clip-text text-transparent">Pets value Calculator</h1>
        <div class="sm:p-6 p-3">
          <div class="score-bar bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl border border-slate-600 sm:px-8 px-3 sm:py-4 py-2 mb-8 grid grid-cols-3 shadow-xl">
            <div class="flex items-center space-x-4">
              <span class="sm:text-6xl text-xl font-bold text-emerald-400">{{ leftValue }}</span>
            </div>
  
            <div class="flex items-center justify-center sm:space-x-8 space-x-1">
              <div class="text-center">
                <div class="text-slate-300 sm:text-xl text-sm font-medium uppercase sm:tracking-wider mb-1">YOUR OFFER</div>
              ...
              <div class="text-center">
                <div class="text-slate-300 sm:text-xl text-sm font-medium uppercase sm:tracking-wider mb-1">THEIR OFFER</div>
              </div>
            </div>
            
            <div class="flex items-center justify-end space-x-4">
              <span class="sm:text-6xl text-xl font-bold text-blue-400">{{ rightValue }}</span>
            </div>
          </div>
  
          <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border border-slate-600 sm:p-8 p-2 py-6 sm:py-8 relative shadow-2xl">
            <div class="grid grid-cols-2 sm:gap-32 gap-16">
              <div class="space-y-4">
                <div class="grid grid-cols-3 sm:gap-3 gap-1">
                  <div
                    v-for="index in 9"
                    :key="`left-${index-1}`"
                    class="trade-slot rounded-lg sm:h-28 h-14 flex items-center justify-center transition-all duration-300 shadow-lg"
                    ...
                    <template v-if="leftItems[index-1]">
                      <div class="text-center">
                        <div class="flex justify-center">
                          <img class="w-full h-full sm:p-2.5 rounded" :src="`https://storage.silverfoxvalues.com/gag/${leftItems[index-1].imageName}`"/>
                        </div>
                      </div>
                    </template>
                    <template v-else-if="index-1 === leftItems.length && index-1 < 9">
                      <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-full sm:w-12 sm:h-12 h-6 w-6 flex items-center justify-center shadow-lg hover:shadow-emerald-500/25 transition-all duration-300">
                ...
                <div class="grid grid-cols-3 sm:gap-3 gap-1">
                  ...
                  >
                    <template v-if="rightItems[index-1]">
                      <div class="text-center">
                        <div class="flex justify-center">
                          <img class="w-full h-full sm:p-2.5 rounded" :src="`https://storage.silverfoxvalues.com/gag/${rightItems[index-1].imageName}`"/>
                        </div>
                      </div>
                    </template>
                    <template v-else-if="index-1 === rightItems.length && index-1 < 9">
                      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-full sm:w-12 sm:h-12 h-6 w-6 flex items-center justify-center shadow-lg hover:shadow-blue-500/25 transition-all duration-300">
                        <svg class="text-white" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M11 13H5v-2h6V5h2v6h6v2h-6v6h-2z"/></svg>
            ...
            <div class="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
              ...
              <div 
                class="rounded-full sm:w-20 sm:h-20 w-14 h-14 flex items-center justify-center border-2 shadow-2xl transition-all duration-300"
                :class="diffBackgroundClass"
              >
                <span class="text-white sm:text-xl text-xs font-bold">{{ difference }}</span>
              </div>
            </div>
            <h1 class="text-center text-lg font-bold mt-3 text-slate-400">Grow a Garden Calculator</h1>
          </div>
        </div>
        <div class="bg-[#111827]/30 text-gray-100 p-6 md:p-10 rounded-2xl shadow-xl border border-yellow-900 mx-auto mt-10">
          <h2 class="text-3xl md:text-4xl font-extrabold text-center mb-4">
            🚨 Attention GAG Traders! 🚨
          </h2>

          <p class="text-lg leading-relaxed mb-4">
            We’ve built a <span class="font-semibold">brand-new GAG Calculator! 💥</span>
          </p>

          <p class="text-base mb-4">
            Previously, it used <span class="font-semibold">Shekel values</span>, but due to inflation and low demand, 
            no one wants Shekels anymore. 😅
          </p>

          <p class="text-base mb-4">
            Our <span class="font-semibold">team of 10</span> behind the scenes has created a 
            <span class="font-semibold">perfect point system</span> for all GAG pets! 🦊✨
          </p>

          <ul class="list-none space-y-2 mb-6">
            <li>✅ Know the perfect value of every pet and trade without taking an L.</li>
            <li>💡 Tip: Use our calculator so you never get any <span class="font-semibold">L</span> in trade.</li>
            <li>💎 Never get scammed on a trade again!</li>
          </ul>
        </div>
      ...
      <div 
        ...
        @click="closeModal"
      >
        <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border border-slate-600 p-6 max-w-xl w-full mx-4 shadow-2xl" @click.stop>
          ...
          <div class="grid sm:grid-cols-4 grid-cols-2 gap-3 max-h-96 overflow-y-auto">
            <div
            ...
            >
              <div class="flex justify-center">
                <img class="w-24 h-24 rounded-lg" :src="`https://storage.silverfoxvalues.com/gag/${item.imageName}`"/>
              </div>
              <div class="text-sm font-medium text-white mt-2">{{ item.name }}</div>
              <div class="text-xs text-emerald-400 font-semibold">{{ item.price }}</div>
            </div>
